...
sed -n -e's|^\([0-9A-F]\{4,5\}\); S; \([0-9A-F]\{4,5\}\); *# |'"$codeLineTemplateS"'|p' $localFile  >> $cppdataFile || exit 1
...
echo 'Writing .cpp data file prefix for stage 3...'
cat << _EOF_ >> $cppdataFile || exit 1
...
echo 'Generating .cpp data file, stage 3...'
sed -n \
...
echo 'Writing .cpp data file suffix for stage 3...'
cat << _EOF_ >> $cppdataFile || exit 1
    { 0, { { 0 }, 0 } }
  };
