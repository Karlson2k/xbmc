...
public:
  ...
  static const std::vector<std::wstring>& getCrtNames()
  { return s_crtNames; }
private:
  std::vector<void*> m_funcPointers;
  std::vector<void*> m_crts; // actually contains HMODULE

  static std::vector<std::wstring> s_crtNames;
};

// Call function in all loaded CRTs
// Function must have same return type and same parameters in all CRTs
#define CALL_IN_CRTS(function,...) ::win32_utils::crts_caller::call_in_all_crts(__STR_MACRO(function),&(function),##__VA_ARGS__)
